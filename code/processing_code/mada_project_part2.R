...
#Load data from 2013 - 2018
#df1318 <- read_sheet###

#write_rds(df2021,"df2021.rds")
...
df1318 <- df1318 %>% select(-weight,-beer,-wine,-liquor,-snack,-meal,-total_food)
...
saveRDS(rhr, file = save_data_location) #clean data with NA dates. Keeping to preserve notes from days off
###
...
rhr %>% ggplot(aes(x=date, y=`rhr(bpm)`))+
  geom_line()+
  geom_smooth(method = "lm")


run_df %>% ggplot(aes(x=datetime, y= avg_pace))+
  geom_point()
...
#Can I see any improvement in my performance in the last 8 years?
run_df %>% ggplot(aes(x=datetime, y=avg_pace))+
  geom_point()+
  geom_smooth(method = "lm")

#From this, it looks like I'm getting slower. This doesn't show the extra distance I go or the type of run
run_df %>% filter(datetime >= "2018-01-01") %>%
  ggplot(aes(x=datetime,y=avg_pace))+
  geom_point()+
  geom_smooth(method = "lm")
...
#That looks better. I guess I could use some harder runs!
run_df %>% filter(run_type != "Recovery") %>%
  filter(distance >= 12) %>%
  filter(datetime >= "2018-01-01") %>%
  ggplot(aes(x=datetime,y=avg_pace))+
...
# Now I want to test if there are correlations between any of my variables
...
#Until I find a better way to coerce my time data (avg_pace specifically), the best way to look for correlations will be through plots. Summaries do not take my current format.

#Garmin Data

#cadence
garminRun %>% ggplot(aes(x=avg_run_cadence, y=avg_pace))+
  geom_point()+
  geom_smooth(method = "lm")
...
garminRun %>% ggplot(aes(x=aerobic_TE, y=avg_pace))+
  geom_point()+
  geom_smooth()

garminRun %>% ggplot(aes(x=avg_hr, y = avg_pace))+
  geom_point()+
  geom_smooth(method = "lm")

garminRun %>% ggplot(aes(x=avg_hr, y = total_ascent))+
  geom_point()+
  geom_smooth(method = "lm")
...
garminRun %>% ggplot(aes(x=aerobic_TE, y=total_ascent))+
  geom_point()+
  geom_smooth(method = "lm")
